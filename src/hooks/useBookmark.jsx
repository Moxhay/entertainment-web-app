...
const useBookmark = () => {
    ...
    const updateCache = (id, bookmarked, type) => {
        ...
        const endpointTrendingName =
            type === 'Movie' ? 'getTrendingMovies' : 'getTrendingSeries'
    ...
    const handleBookmarked = async ({ id, currentBookmarkedState, type }) => {
        ...
        try {
            if (currentBookmarkedState) {
                ...
                await postBookmarked({ id, userId, contentType })
            }
        } catch (err) {
            setError(
    ...
    const postBookmarked = async ({ id, userId, contentType }) => {
    ...
    const deleteBookmarked = async ({ userId, id, contentType }) => {
        ...
        if (!deleteBookmarked.success) {
            throw new Error(
                deleteBookmarked.error || 'Failed to delete bookmark'
            )
        }
    }
